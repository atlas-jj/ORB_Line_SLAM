...
namespace ORB_SLAM2
{
  ...
  OrbLine::OrbLine(cv::Mat _previousImage, cv::Mat _currentImage,
    cv::Mat _previousTWC, cv::Mat _currentTWC, double _fx, double _fy, double _cx, double _cy)
    {
  ...
  std::vector<std::vector<cv::Point3d> >  OrbLine::GenerateLineMapPoints()
  {
	...
	imshow("view",currentKFImage);
       ...
       // cout<<"F"<<endl<<F<<endl;
        ...
        std::cout<<"lines detected  "<<crosKl1.size()<<std::endl;
	int newPointsCreated=0;
	  ...
	  std::vector<cv::Point3d> outputPoints;
          ...
          //test , select one point in line1, find its best matches using epipolar constraints
           ...
           startPairs=refineSearchPoints(startP1, rightLine, F);
           endPairs=refineSearchPoints(endP1, rightLine, F);
           if(startPairs.size()>1)
           {
  ...
  std::vector<cv::Point2d> OrbLine::refineSearchPoints(cv::Point2d pointInLine, cv::Mat rightLine, cv::Mat F)
  {
